...
exports.createBootcamp = asyncHandler(async (req, res, next) => {
  ...
  // Check for pubished bootcamp
  const publishedBootcamp = await Bootcamp.findOne({user: req.user._id});

  // If the user if not an admin, they can only add one bootcamp
...
exports.BootcampPhotoUpload = asyncHandler(async (req, res, next) => {
  ...
  // // Create custom filename with original name and extension
  // file.name =`photo_${path.basename(file.name, path.parse(file.name).ext)}_${bootcamp._id}${path.parse(file.name).ext}`;
